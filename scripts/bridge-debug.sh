...
get_addresses() {
    if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "sequencer-poc-relayer"; then
        docker exec sequencer-poc-relayer cat /config/addresses.json 2>/dev/null
    fi
}
...
echo "=== Relayer Status ==="
RELAYER_STATUS=$(docker compose -f /Users/max/claude/gateway/rnd/sequencer-poc/docker-compose.yml ps hyperlane-relayer --format '{{.State}}' 2>/dev/null || echo "unknown")
echo "Relayer container: $RELAYER_STATUS"

# Check for recent errors
echo ""
echo "=== Recent Relayer Errors (last 50 lines) ==="
docker compose -f /Users/max/claude/gateway/rnd/sequencer-poc/docker-compose.yml logs hyperlane-relayer --tail 50 2>&1 | grep -i -E "error|warn|fail" | tail -10 || echo "No recent errors"
